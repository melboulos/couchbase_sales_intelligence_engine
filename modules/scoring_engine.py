def calculate_coi(row):
    ...
    business_model = str(row.get("business_model", "Unknown"))

    company_signal = int(row.get("company_signal_score", 0))

    technology_score = int(row.get("technology_score", 0))

    company_size = str(row.get("company_size", "Unknown"))

    engineering = str(row.get("engineering_signal", "Unknown"))

    revenue = str(row.get("revenue_signal", "Unknown"))

    ai = str(row.get("ai_initiatives", "Unknown"))
    ...
    breakdown["overall_coi"] = min(score,100)
