...
def export_account_intelligence(accounts):
    ...
    for _, row in accounts.iterrows():
        record = {
            "account_name": clean_value(row.get("Account Name", "")),
            "industry": clean_value(row.get("industry", "")),
            "overall_coi": clean_value(row.get("overall_coi", 0)),
            "priority_tier": clean_value(row.get("priority_tier", "")),
            "business_model": clean_value(row.get("business_model", "")),
            "workloads": clean_value(row.get("workloads", [])),
            "technology_signals": {
                "cloud_signal": clean_value(row.get("cloud_signal", "")),
                "database_signal": clean_value(row.get("database_signal", "")),
                "ai_signal": clean_value(row.get("ai_signal", ""))
            },
            "sales_intelligence": {
            ...
            "llm_validation": {
                "llm_run_id": clean_value(row.get("llm_run_id", "")),
                "llm_validation": clean_value(row.get("llm_validation", ""))
            }
        }
    ...
    print(f"Saved intelligence JSON: {OUTPUT_FILE}")
